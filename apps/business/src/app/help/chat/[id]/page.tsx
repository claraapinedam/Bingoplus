    ...
    <div style={{ display: 'flex', gap: 2 }}>
      {[1, 2, 3, 4, 5].map((n) => (
        <button
          ...
          disabled={readOnly}
          onClick={() => onChange?.(n)}
          style={{ background: 'none', border: 'none', padding: 0, cursor: readOnly ? 'default' : 'pointer', fontSize: 22, color: n <= value ? '#f5a623' : '#e0e4ea' }}
        >
          ★
        </button>
      ))}
        ...
        {chat.status === 'OPEN' ? (
          <>
            ...
            <div className="bingo-card" style={{ marginTop: 16 }}>
              ...
              />
              <button className="bingo-button secondary" style={{ marginTop: 8, width: 'auto', padding: '0 20px' }} onClick={endChat} disabled={rateBusy}>
