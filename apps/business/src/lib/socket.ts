...
/** One shared socket per tab. `auth` is a callback (not a plain object) so every reconnect —
 * including after a token refresh — sends the *current* token, not whatever was in localStorage
 * at first connect. */
