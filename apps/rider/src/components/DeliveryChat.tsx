 ...
 * (short-polling — see the API's SupportChatsService header comment for why), this reuses the
 * DeliveryGateway `delivery:{id}` socket room the host page (delivery detail) already subscribes
 * to for status updates — this component only adds its own `delivery.chat.message` listener on
 * top of that existing subscription, it does not join/leave the room itself. REST GET is still
 * the source of truth for history (first load, and catch-up after a reconnect gap).
 */
...
export default function DeliveryChat({ deliveryId, active }: DeliveryChatProps) {
  ...
  useEffect(() => {
    apiFetch<DeliveryChatMessage[]>(`/rider/deliveries/${deliveryId}/chat/messages`)
      .then(addMessages)
      .catch(() => undefined);
  ...
  async function send() {
    ...
    setError(null);
    try {
      const message = await apiFetch<DeliveryChatMessage>(`/rider/deliveries/${deliveryId}/chat/messages`, {
        method: 'POST',
          ...
          <div style={{ minHeight: 160, maxHeight: 280, overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: 8 }}>
            {messages.length === 0 && (
              <div style={{ fontSize: 13, color: '#7f8ea3' }}>
                {active ? 'Escríbele al cliente cuando quieras.' : 'No hay mensajes en esta entrega.'}
              </div>
            )}
            {messages.map((m) => (
              <div
                key={m.id}
                style={{
                  alignSelf: m.senderType === 'RIDER' ? 'flex-end' : 'flex-start',
                  background: m.senderType === 'RIDER' ? 'var(--bingo-teal)' : '#f2f4f7',
                  color: m.senderType === 'RIDER' ? 'white' : 'var(--bingo-navy)',
                  borderRadius: 12,
                  padding: '8px 12px',
                  maxWidth: '80%',
                  fontSize: 13,
